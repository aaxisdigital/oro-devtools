...
interface StorageBrowserOptions {
    _sourceElement: any;
    listUrl: string;
    previewUrl: string;
    rawUrl: string;
...
type SortKey = 'type' | 'name' | 'size' | 'modified' | 'created' | 'ownerUser' | 'ownerGroup';
...
class StorageBrowserComponent extends BaseComponent {
    ...
    private createFolderUrl!: string;
    private uploadUrl!: string;
    private deleteUrl!: string;

    private lastListing!: Listing | null;
    ...
    initialize(options: StorageBrowserOptions): void {
    ...
    private hasDotDot(path: string): boolean {
    ...
    private onToggleFilter(event: any): void {
    ...
    private onPathEnter(value: string): void {
    ...
    private navigate(path: string): void {
        ...
        $.ajax({url: this.listUrl, method: 'GET', data: {path}})
            .done((response: Listing & {success: boolean; message?: string}) => {
                if (!response.success) {
                    if (this.handleLoadFailure(response.message)) {
                        return;
                    }
                    messenger.notificationFlashMessage('error',
                        response.message || __('aaxis.devtools.storage_browser.list_error'));
     ...
     */
    private handleLoadFailure(message?: string): boolean {
    ...
    private renderLoading(): void {
        this.$el.find('[data-role="content"]').empty().append(
            $('<div/>', {'class': 'aaxis-fs__loading'})
    ...
    private onSortClick(event: any): void {
        event.preventDefault();
        const key = String($(event.currentTarget).data('sort')) as SortKey;
        if (this.sortKey === key) {
    ...
    private sortHeader(key: SortKey, label: string): any {
    ...
    private appendRow($body: any, entry: FsEntry, index: number): void {
        const $tr = $('<tr/>', {
            'class': 'aaxis-fs__row',
            'data-role': 'row',
            'data-index': index,
        ...
        const $name = $('<td/>', {'class': 'aaxis-fs__name-cell'}).append(
            $('<span/>', {'class': 'fa ' + iconClass + ' aaxis-fs__entry-icon', 'aria-hidden': 'true'})
        );
    ...
    private onRowDblClick(event: any): void {
        if ($(event.target).closest('[data-role="preview-btn"], [data-role="download-btn"], [data-role="delete-btn"]').length) {
            return;
        }
    ...
    private openEntry(entry?: FsEntry): void {
        if (!entry) {
            return;
        }
        if (entry.type === 'dir') {
    ...
    private onCreateFolder(event: any): void {
        event.preventDefault();
        if (!this.writable) {
            return;
        }
        ...
        $.ajax({
            ...
            .done((response: {success: boolean; message?: string}) => {
                if (!response.success) {
            ...
            .fail((jqXhr: any) => this.flashAjaxError(jqXhr));
    }

    private onUploadClick(event: any): void {
        event.preventDefault();
        if (!this.writable) {
            return;
        }
    ...
    private onUploadChange(event: any): void {
    ...
    private flashAjaxError(jqXhr: any): void {
        const message = (jqXhr.responseJSON && jqXhr.responseJSON.message)
    ...
    private openFile(path: string, name: string): void {
        ...
        $.ajax({url: this.previewUrl, method: 'GET', data: {path}})
            .done((response: {success: boolean; message?: string; binary?: boolean; content?: string; truncated?: boolean}) => {
                ...
                if (!response.success) {
                    ...
                    messenger.notificationFlashMessage('error',
                        response.message || __('aaxis.devtools.storage_browser.preview_error'));
                    return;
                }
                if (response.binary) {
    ...
    private openMediaTab(name: string, path: string, kind: 'image' | 'pdf'): void {
        this.previewSeq += 1;
        const id = 'tab-' + this.previewSeq;
    ...
    private onTabClose(event: any): void {
        event.preventDefault();
        event.stopPropagation();
    ...
    private renderCode(tab: BrowserTab): void {
        const $content = this.$el.find('[data-role="content"]').empty();
        if (tab.loading) {
        ...
        if (tab.mode) {
            try {
                (CodeMirror as any).runMode(String(tab.text || ''), tab.mode, pre);
            } catch (e) {
    ...
    private renderMedia(tab: BrowserTab): void {
        const $content = this.$el.find('[data-role="content"]').empty();
